    ...
    public class CompatibilityService : ICompatibilityService
    {
       ...
       int elementId)
        ...
        private async Task<string> GetRecommendedColor(int elementId)
        {
            ...
            var recommendedColor = recommendations
                .GroupJoin(breeds, r => r.BreedId, b => b.BreedId, (r, b) => new { Recommendation = r, Breeds = b })
                .SelectMany(x => x.Breeds.DefaultIfEmpty(), (x, b) => new { x.Recommendation, Breed = b })
                .GroupBy(x => x.Breed.Color)
                .OrderByDescending(g => g.Count())
                .FirstOrDefault();
